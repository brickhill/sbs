...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        company = qs[0]
        print(company.name)
        qs = Price.objects.filter(Q(company=company)&Q(period="D")).values("price_time","high", "low", "open", "close", "volume")
        df = read_frame(qs)
        ...
        print(df.head())
        chart = Chart(options['symbol'], df, period="year")

...
class Chart:
  ...
  def __init__(self, symbol, df, period="year", candles=True, centre=None):
    self.traces = 0
    self.period = period
    self.symbol = symbol
    self.df = df
    ...
    if str(self.period).isdigit():
      ...
      self.df = self.df[self.df.index >= pytz.utc.localize(dt.datetime.now()) - relativedelta(years=1)]
    else:
      period = -1
  ...
  def show_plot_browser(self,width=1400,height=800, nticks=8, line_traces=['MA10', 'MA30']):
  ...
  def create_candle_fig(self):
    ...
    if self.candles == True:
      candlechart = self.fig.add_trace(go.Candlestick(
        x=self.df.index,
        open=self.df.open,
        high=self.df.high,
        low=self.df.low,
...
#       self.fig.add_trace(
#         go.Bar(x=self.df.index, y=self.df.volume), row=3, col=1
#       )
 
      self.fig.add_trace(go.Scatter(x=self.df.index,y=self.df.BOLL_low,
        fill = None,
        line=dict(width=1),line_shape="spline",),row=2,col=1)
      ...
      self.fig.add_trace(go.Scatter(x=self.df.index,y=self.df.BOLL_high,
        fill = "tonexty",
        line=dict(width=1),line_shape="spline",
        fillcolor='rgba(55, 150, 122, 0.1)'
        ),row=2,col=1)

#   def addTimeString(self):
#     self.df['sDate']=[dt.datetime.strftime(x, "s%y-%m-%d") for x in self.df.loc]
